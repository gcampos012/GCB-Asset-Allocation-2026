...
tickers_usd = ["VT", "VTI","GLD"]
tickers_brl = ["DIVO11.SA"]
tickers = tickers_brl + tickers_usd
...
usdbrl = yf.download("BRL=X", start=dez_anos_atras, end=hoje)["Close"].reindex(dados.index, method="ffill")
dados

# %%
for ticker in tickers_brl:
    dados[ticker] = dados[ticker] / usdbrl["BRL=X"]
dados["SELIC"] = dados["SELIC"] / usdbrl["BRL=X"]
dados
